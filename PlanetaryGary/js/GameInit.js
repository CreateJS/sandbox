...
(function() {
	...
	uiStage.canvas.width = window.innerWidth;
	uiStage.canvas.height = window.innerHeight;
	...
	function handleResize() {
		if (uiStage.canvas.style.visibility == "visible") {
			...
			menu.x = canvasW - 822>>1;
			menu.y = (canvasH * 0.75) - 165 | 0;

			line.graphics.clear().f("#B64444").dr(0, 0, canvasW, line.height);
			pink.graphics.clear().f("#eb9c9b").dr(0, 0, canvasW, pink.height);
			line.cache(0, 0, canvasW, line.height);
			pink.cache(0, 0, canvasW, pink.height);
			line.y = (canvasH * 0.75);
			pink.y = line.y - pink.height;
			currentStage.update();
		}else {
	...
	function addBGDecorations() {
		//Since frame was removed BG needs to be adjusted to fill in space
		menu.regX = 0;
		menu.regY = 552.6>>1;
		menu.x = canvasW - 822>>1;
		menu.y = (canvasH * 0.75) - 165 | 0;
		...
		line.height = 3;
		line.graphics.f("#B64444").dr(0, 0, canvasW, line.height);
		line.cache(0, 0, canvasW, line.height);
		line.regY = line.height/2;
		line.y = (canvasH * 0.75);

		pink = new createjs.Shape();
		pink.height = 131;
		pink.graphics.f("#eb9c9b").dr(0, 0, canvasW, pink.height);
		pink.cache(0, 0, canvasW, pink.height);
		uiStage.addChildAt(line, 0);
		uiStage.addChildAt(pink, 0);
		pink.y = line.y - pink.height;
	}
